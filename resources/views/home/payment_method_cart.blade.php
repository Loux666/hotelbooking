...
<div class="payment-methods-container" style="max-width:600px; margin: 40px auto; padding:20px; border:1px solid #ddd; border-radius:8px;">
    ...
    <p>Đơn của bạn: <strong>#{{ $booking->id }}</strong></p>

    @if ($discount > 0)
        <p>Giá gốc: <del>{{ number_format($booking->total_price + $discount, 0, ',', '.') }}đ</del></p>
        <p>Ưu đãi: <span class="text-success">-{{ number_format($discount, 0, ',', '.') }}đ</span></p>
    @endif

    <p><strong>Tổng tiền cần thanh toán: {{ number_format($booking->total_price, 0, ',', '.') }}đ</strong></p>
    
    

    <form action="{{ route('payment.init.cart') }}" method="POST">
        @csrf
        <input type="hidden" name="booking_id" value="{{ $booking->id }}">
        <input type="hidden" name="cache_key" value="cart-booking-{{ $booking->id }}">

        <div class="mb-3">
            <label><input type="radio" name="method" value="vnpay" checked> Thanh toán qua VNPAY</label><br>
            
            <label><input type="radio" name="method" value="offline"> Thanh toán khi đến nhận phòng</label>
        </div>
